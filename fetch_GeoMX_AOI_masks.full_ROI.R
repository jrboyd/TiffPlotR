...
tf = tiff_files$TomatoRed_PanCk_CD45_3124.ome_.tiff
#
#
...
# tf = "/mnt/c/project_data/UVMMIC-RA-13113/LXlI4oj06AJKJfOF3Ug9/TomatoRed_PanCk_CD45_3124.ome_.tiff"
# tf = "/mnt/c/project_data/UVMMIC-RA-13113/LXlI4oj06AJKJfOF3Ug9/TomatoRed_CD45_PanCK_3124.ome_.tiff"
...
for(tf in tiff_files[2:3]){
    ...
    cache_dir = "cache_mask_results.full_ROI"
    dir.create(cache_dir)
    mask_points =  ann$mask_points

#### add in background points ####
    ...
    poly_pts = shape_contained_points(createPolygonsFromAnnotations(ann$polygons))
    elli_pts = shape_contained_points(createEllipsesFromAnnotations(ann$ellipses))

    roi_points = rbind(poly_pts, elli_pts) %>% tibble
    roi_points = roi_points %>% rename(i = x, j = y, ROI_ID = name)

    mask_points %>% head
    mask_points$node_type = NULL
    mask_points$ID = NULL
    mask_points$FillColor = NULL
    mask_points = mask_points %>% tibble

    roi_points = merge(mask_points, roi_points, by = c("ROI_ID", "i", "j"), all.y = TRUE)
    roi_points %>% head
    roi_points = roi_points %>% mutate(Text = ifelse(is.na(Text), "background", Text))
    nrow(tmp)
    nrow(mask_points)
    nrow(roi_points)

    mask_points = roi_points
