...
gather_channel_quantiles = function(tf, max_full_fetch = 100e6, query_probs = .query_probs(), workers = "auto"){
  ...
  for(res_i in seq(max_res, 1)){
    ...
    message("using fetch mode ", fetch_mode, " @resolution ", res_i)
    ...
    switch (
      ...
      FULL = {
        ...
        colnames(quant_res) = seq(ncol(quant_res))
        if(preserve_rects){
          ...
          #get .99 quantiles for all rectangles, only used for ranking by .select_q
      ...
      LOW = {
        ...
        .run_todo_i = function(todo_i){
          ...
          q99_per_range = lapply(seq(nrow(sample_ranges)), function(i){
            xs = seq(sample_ranges[i, "xmin"], sample_ranges[i, "xmax"])
            ys = seq(sample_ranges[i, "ymin"], sample_ranges[i, "ymax"])
            # run 3d (channel stack) or 2d mode
            if(length(dim(arr)) == 3){
        ...
        if(workers == 1){
          todo_result = lapply(seq(n_todo), .run_todo_i)
        }else{
        ...
        todo_result.by_channel = split(todo_result, future_todo$chan_i)
        
        chan_result = todo_result.by_channel[[1]]
        .process_channel = function(chan_result){
          #separate list outputs
          q99_per_range_list = lapply(chan_result, function(x){
            x$q99_per_range
          })
          
          sample_ranges_list = lapply(chan_result, function(x){
            x$sample_ranges
          })
          
          fetch_contents = lapply(chan_result, function(x){
            x$fetch_content
          })
          
          # select best rect
          sample_ranges = dplyr::bind_rows(sample_ranges_list)
          # tr = do.call(TiffRect, sample_ranges)
          # tr = .mult_TiffRect(tr, 2^4)
          #img5@plots$rgb %>% shape_annotate(tr, fill = "#00550042")
          
          q99_per_range = unlist(q99_per_range_list)
          
          q99_per_range.mat = matrix(unlist(q99_per_range), nrow = nrow(sample_ranges), byrow = TRUE)
          ...
          list(fetch_contents = unlist(fetch_contents), representative_ranges = representative_ranges.i[[1]])
        }
        
        
        processed_channel = lapply(todo_result.by_channel, .process_channel)
        
        fetch_contents.by_channel = lapply(processed_channel, function(x)x$fetch_contents)
        representative_ranges.i = lapply(processed_channel, function(x)x$representative_ranges)
        
        quant_res = lapply(fetch_contents.by_channel, function(fc){
          .gather_quantiles_full_res(
            unlist(fc),
            probs = query_probs
          )
        })
        ...
        representative_ranges.by_channel = representative_ranges.i
...
.query_probs = function(){
  unique(sort(c(seq(0, 100, by = .1), seq(99,100, by = .01), seq(99.9,100, by = .001))))/100
}
