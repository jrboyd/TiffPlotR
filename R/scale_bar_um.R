...
add_um_scale_bar <- function(p,
    ...
    if (!is.numeric(label_offset_fraction) || length(label_offset_fraction) != 1L || !is.finite(label_offset_fraction)) {
        stop("label_offset_fraction must be a single numeric value")
    }
    ...
    x_start <- if (xor(is_left, x_rev)){
        x_min + width_px * pad_fraction
    }  else {
        x_max - width_px * pad_fraction - bar_px
    }
    x_end <- x_start + bar_px

    y_bar <- if (xor(is_bottom, y_rev)){
        y_min + height_px * pad_fraction
    } else {
        y_max - height_px * pad_fraction
    }

    # Place labels away from the nearest edge so they remain legible.
    label_direction <- if (xor(is_bottom, y_rev)){
        1
    }  else {
        -1
    }
    y_label <- y_bar + label_direction * (height_px * (label_offset_fraction - ifelse(is_bottom, .03, 0)))
    ...
    p +
        ...
        ggplot2::annotate(
            ...
            color = text_color,
            size = text_size,
            vjust = ifelse(label_direction > 0, 1, -1),
            hjust = .5
        )
}
#
